...
# -*- coding: utf-8 -*-
"""
horizon_3H.py — Modèle final XGBoost  |  Station P  |  H+3h
=============================================================
Prédiction directe du débit à l'horizon H=3h.

Features retenues : G6 (baseline V1) + G12 (lags ponctuels + baseflow)
  → sélectionnées par ablation study (modele_XGBoost_finalV1 à V4)
  → G12 est la seule config à améliorer le CV vs baseline (-0.72 MAE)
  → soil moisture (V3) et features V2/G9 (V2) ont dégradé le CV

Pipeline :
  1. Chargement et nettoyage des données
  2. Feature engineering (G6 + G12)
  3. GridSearch temporel (train fixe → val fixe)
  4. Robustesse : sliding window CV avec 6 configs de fenêtres
  5. Modèle final sur tout le CV → évaluation test
  6. Métriques complètes + graphiques (Bland-Altman débit réel en X)
"""
...
HORIZON          = 3           # heures
W_TEST_FINAL     = 8 * 7 * 24  # 8 semaines de test final (≈ nov-jan)
...
# ── G6 — Features baseline (sélectionnées par V1) ────────────
...
df["prev_pluie_cumul"]  = df["prev_pluie_1h"] + df["prev_pluie_2h"] + df["prev_pluie_3h"]
...
if HAS_SCIPY:
    ...
    try:
        ...
        print(f"  Cyclic fit  R2={r2_fit:.3f}  beta={popt[0]:.1f}  alpha={popt[1]:.1f}  phi={popt[2]:.3f}")
        CYCLIC_OK = True
...
print(f"  G6 : {30} features baseline  |  seuil crue P90 = {SEUIL_CRUE:.1f} m³/h")

# ── G12 — Lags ponctuels + baseflow (sélectionnées par V4) ───
...
print(f"  G12 : 20 features ciblées crues retardées")
...
G6_FEATS = [
    ...
    "prev_pluie_1h","prev_pluie_2h","prev_pluie_3h","prev_pluie_cumul",
...
# Fenêtre fixe pour le GridSearch : évite la fuite temporelle
W_GS_TRAIN = 10 * 7 * 24
W_GS_VAL   =  2 * 7 * 24
...
# n_estimators fixé haut + early stopping
N_EST_MAX = 800
EARLY_STOP = 30

print(f"\n  Grid : {len(PARAM_GRID['max_depth'])} × {len(PARAM_GRID['learning_rate'])} × "
      f"{len(PARAM_GRID['subsample'])} × {len(PARAM_GRID['colsample_bytree'])} = "
      f"{3*3*2*2} combinaisons\n")
...
for md, lr, sub, col in product(
    ...
    gs_results.append({"max_depth": md, "lr": lr, "sub": sub, "col": col,
                        "mae": mae, "r2": r2, "n_est": n_est})
    if mae < best_mae:
        best_mae  = mae
...
# n_estimators du GridSearch est calé sur 1 fenêtre (10w train, 2w val).
# En production le modèle s'entraîne sur 5175h → plus de données → plus d'arbres.
# On applique un facteur d'échelle proportionnel à la taille du train, plancher à 300.
scale_factor = len(df_cv) / W_GS_TRAIN   # ≈ 5175/1680 ≈ 3.1
...
print(f"  MAE val GridSearch = {best_mae:.2f} m³/h")

best_n_est = best_n_est_scaled

# ══════════════════════════════════════════════════════════════
#  4. ROBUSTESSE — SLIDING WINDOW CV  (6 configs de fenêtres)
# ══════════════════════════════════════════════════════════════
print("\n" + "=" * 65)
...
WINDOW_CONFIGS = [
    (8  * 7 * 24,  1 * 7 * 24,  1 * 7 * 24),   # 8w train, 1w val, 1w step
    (8  * 7 * 24,  2 * 7 * 24,  2 * 7 * 24),   # 8w train, 2w val, 2w step
    (10 * 7 * 24,  1 * 7 * 24,  1 * 7 * 24),   # 10w train, 1w val, 1w step
    (10 * 7 * 24,  2 * 7 * 24,  2 * 7 * 24),   # 10w train, 2w val, 2w step  ← config V1-V4
    (12 * 7 * 24,  2 * 7 * 24,  2 * 7 * 24),   # 12w train, 2w val, 2w step
    (12 * 7 * 24,  3 * 7 * 24,  3 * 7 * 24),   # 12w train, 3w val, 3w step
]
...
for (w_tr, w_va, step) in WINDOW_CONFIGS:
    ...
    n_f  = len(df_r)
    if n_f == 0:
        print(f"  {w_tr//168:>5}sem {w_va//168:>3}sem  {step//168:>3}sem | pas assez de données")
        continue
    ...
    print(f"  {w_tr//168:>5}sem {w_va//168:>4}sem {step//168:>4}sem | {n_f:>5} | "
    ...
    robustness_results.append({
        ...
        "folds": n_f, "fold_maes": df_r["mae"].values
    })
...
errors   = actuals - preds_te   # positif = sous-estimation
...
diffs  = preds_te - actuals   # positif = sur-estimation
bias   = diffs.mean()
std_d  = diffs.std()
...
pct_sous = (errors > 0).mean() * 100   # sous-estimations
...
print(f"""
  ...
  │  R²   test     = {r2_te:>6.3f}                          │
  │  MAE naïf      = {mae_naif:>6.2f} m³/h  (persistence H=3)  │
...
ax2.bar(mae_h.index, mae_h.values, color="#3498db", alpha=0.8)
...
# ── 6.3 Robustesse CV — boxplot MAE par config ────────────────
...
fi_top = fi.head(20)
colors_fi = ["#27ae60" if f in G12_FEATS else "#3498db" for f in fi_top["feature"]]
...
patch_g6  = mpatches.Patch(color="#3498db", alpha=0.7, label="G6 baseline")
...
patches7 = [
    ...
    mpatches.Patch(color="#f39c12", alpha=0.6, label=f"Moyen"),
    mpatches.Patch(color="#e74c3c", alpha=0.6, label=f"Élevé"),
...
out_fig = OUT / "horizon_3H_evaluation.png"
plt.savefig(out_fig, dpi=150, bbox_inches="tight")
print(f"  -> {out_fig}")

print("\n" + "=" * 65)
print("FIN horizon_3H.py")
print("=" * 65)
